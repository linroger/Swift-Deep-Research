...
1. **深度研究** —— DeerFlow 进行多轮背景调查，产出研究档案以及一组真实世界角色（人物、公司、机构），附带立场、角色与影响力。实时控制台逐行流式展示研究过程。
2. **本体** —— 流水线决定这个问题需要关注哪些实体类型与关系类型。
3. **知识图谱** —— 实体被抽取进 Zep 时序知识图谱，并以**原生交互式力导向图**（[Grape](https://github.com/li3zhen1/Grape) 包）渲染：可平移、缩放、拖拽节点、开关标签；点按任意节点弹出检视面板，展示其类型、摘要与全部关系。
4. **Agent 构建** —— 图谱中的每个角色都变成一个拥有人格、记忆与发帖行为的自治 LLM Agent。
5. **社会模拟** —— OASIS 让这个 Agent 社会在模拟的 **Twitter 与 Reddit** 上运行数十轮。阶段视图展示按平台的遥测数据、逐轮行为柱状图，以及每一条发帖、评论、点赞的实时动态流。
6. **预测报告** —— ReAct 报告 Agent 回读整个模拟（可以通过工具调用「采访」单个 Agent），写出最终预测。之后你还可以**与报告对话**：追问会带着完整的模拟上下文运行。
...
- **受监管的后端。** App 自行拉起并健康检查 Flask 后端，把失败分类成可操作的提示（端口被占、虚拟环境损坏、`.env` 配置缺失），并提供一键**环境修复**。
