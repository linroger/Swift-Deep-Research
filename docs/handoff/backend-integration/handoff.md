...
- 2026-06-16 (ship): main fast-forwarded to the branch and pushed (origin d041b10..72f8c3c); branch pushed. v2.0 Release `.app` built (build 2), ad-hoc signed, **smoke-tested — launches out of the box, no crash, codesign valid**. DMG packaged: `release/Swift-Deep-Research-2.0.dmg` (28MB) + delivered to user. GitHub Release v2.0.0 created (draft id 339978296). UPLOAD GOTCHA: this sandbox stalls large outbound uploads over HTTP/2 (gh hangs at 0 bytes) and throttles HTTP/1.1 to ~21KB/s, so the 28MB asset uploads via `curl --http1.1 -m 2400` (~25 min) directly to uploads.github.com, then `gh release edit v2.0.0 --draft=false` publishes (creates the tag). Background job bxmhhfn1j does upload→publish→verify. To re-do manually: the dmg + release id 339978296 are the inputs.
